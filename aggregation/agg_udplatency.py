...
conn = sqlite3.connect(raw_data_path+"curr_udplatency.db")

def process_month(start_date, end_date):
    print("Processing udplatency for "+start_date+" "+end_date, flush=True)
    df_months_curr = pd.read_sql_query("select * from curr_udplatency "
    ...
    if not df_months_curr.empty :
        ...
        df_months_curr.to_csv(agg_csv_path+"udplatency/agg_udplatency_" + start_date + ".csv", index=False, header=None)
        print('Finished udplatency for '+start_date, flush=True)

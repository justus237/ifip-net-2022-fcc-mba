...
conn = sqlite3.connect(raw_data_path+"curr_httpgetmt6.db")

def process_month(start_date, end_date):
    print("Processing httpgetmt6 for "+start_date+" "+end_date, flush=True)
    df_months_curr = pd.read_sql_query("select * from curr_httpgetmt6 "
    ...
    if not df_months_curr.empty :
       ...
       .rename(columns={'fetch_time':'med_fetch_time', 'bytes_total':'med_bytes_total','bytes_sec':'med_bytes_sec', 'successes':'sum_suc', 'failures':'sum_fail'}))
        ...
        df_months_curr.to_csv(agg_csv_path+"httpgetmt6/agg_httpgetmt6_" + start_date + ".csv", index=False, header=None)
        print('Finished httpgetmt6 for '+start_date, flush=True)
